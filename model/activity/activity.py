...
class Activity(Sandbox, Generic[T]):
    def __init__(self, seed = 0, uid = None):
        super().__init__(seed=seed, uid=uid)
        self.__dict__[_R_KEY] = []
        self.__dict__[_S_KEY] = []
        self.__dict__[_D_KEY] = []
        self.__dict__[_F_KEY] = []
    ...
    @property
    def r_loads_requested_start(self) -> List[T]:
        return self.__dict__[_R_KEY]

    @property
    def s_loads_started(self) -> List[T]:
        return self.__dict__[_S_KEY]

    @property
    def d_loads_ready_finish(self) -> List[T]:
        return self.__dict__[_D_KEY]

    @property
    def f_loads_finished(self) -> List[T]:
    ...
    def signal_start(self, entity: T) -> None:
        ...
        if entity not in self.__dict__[_R_KEY]:
            self.__dict__[_R_KEY].append(entity)
