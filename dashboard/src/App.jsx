...
export default function App() {
    ...
    useEffect(() => { async function load() { try { const [re, ri, or] = await Promise.all([fetch(`${BASE}restaurant.csv`).then(r => { if (!r.ok) throw new Error('restaurant.csv'); return r.text() }), fetch(`${BASE}rider.csv`).then(r => { if (!r.ok) throw new Error('rider.csv'); return r.text() }), fetch(`${BASE}order.csv`).then(r => { if (!r.ok) throw new Error('order.csv'); return r.text() })]); setRestaurants(readCsv(re)); const rr = readCsv(ri); if (rr[0]) setRider(rr[0]); const oo = readCsv(or); setOrder({ count: Number(oo.find(r => r.label === 'order_nums')?.value ?? 0), avg: Number(oo.find(r => r.label === 'average_waiting_time')?.value ?? 0) }) } catch (e) { setError(`Failed to load simulation output: ${e.message}`) } } load() }, [])
